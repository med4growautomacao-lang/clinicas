-- Painel Comercial — Comparecimento por DATA DA CONSULTA.
-- byStatus (realizado/compareceu/faltou/cancelado) e attendedConsults passam a ser ancorados em
-- a.date (data em que a consulta acontece/foi realizada), não em a.created_at (data da marcação).
-- Assim "Consultas Realizadas" e a seção Comparecimento & Perdas refletem o que ocorreu no período.
-- Agendamentos Gerados (v_appt_total) continua por created_at. Faturamento Real (ft.date) já é por
-- data da consulta (finalize_appointment grava ft.date = a.date). Mantém finance.defaultTicket.
