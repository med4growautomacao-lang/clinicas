-- Painel Comercial — seção SLA & Velocidade reescrita por CICLOS DE RESPOSTA:
--   ciclo = mensagem do lead (inbound) seguida da próxima resposta do agente (outbound),
--   medido em MINUTOS ÚTEIS (calc_business_minutes, mesma régua do contador de SLA).
-- Métricas (recortadas pela janela de CONVERSÃO + coorte ENTRADA + origem + agente):
--   • firstResponseMin = MEDIANA do 1º ciclo de cada lead      ("Tempo da 1ª resposta")
--   • responseMin      = MEDIANA de TODOS os ciclos            ("Tempo de resposta")
--   • breaches         = nº de ciclos acima da meta            ("Estouros de SLA")
--   • overBreachMin    = MEDIANA de (ciclo − meta) nos estouros ("Tempo estourado")
-- Durações em mediana (robustas a outliers de atendimento). Contagem = count.
-- Substitui a 1ª-resposta antiga (a partir de created_at) e remove "Aguardando resposta".
-- Fallback p/ tempo corrido quando a clínica não tem business_hours configurado.
...
BEGIN
  ...
  -- ===== SLA & velocidade: config (meta + horário comercial) =====
  SELECT sla_minutes, business_hours, csat_type INTO v_sla_minutes, v_bh, v_csat_type
  ...
  WITH stream AS (
    SELECT cm.lead_id, cm.created_at,
      CASE
  ...
  lagged AS (
    SELECT lead_id, created_at, kind,
  ...
  cyc AS (
    SELECT lead_id, prev_at AS in_at, created_at AS out_at,
      CASE WHEN v_has_bh
        THEN calc_business_minutes(prev_at, created_at, v_sh, v_sm, v_eh, v_em, v_days)
        ELSE GREATEST(0, ROUND(EXTRACT(EPOCH FROM (created_at - prev_at)) / 60.0))::int
      END AS biz
    FROM lagged WHERE kind = 'out' AND prev_kind = 'in'
  ),
  firsts AS (
    SELECT DISTINCT ON (lead_id) lead_id, biz FROM cyc ORDER BY lead_id, in_at
  )
  SELECT
    COALESCE((SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY biz) FROM firsts), 0),
    COALESCE((SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY biz) FROM cyc), 0),
    COALESCE((SELECT COUNT(*) FROM cyc WHERE v_sla_minutes > 0 AND biz > v_sla_minutes), 0),
    COALESCE((SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY (biz - v_sla_minutes)) FROM cyc WHERE v_sla_minutes > 0 AND biz > v_sla_minutes), 0)
  INTO v_median_first_response, v_avg_response, v_sla_breaches, v_avg_over_breach;
  ...
  RETURN jsonb_build_object(
    ...
    'sla', jsonb_build_object(
      'firstResponseMin', ROUND(COALESCE(v_median_first_response,0),1),
      'responseMin', ROUND(COALESCE(v_avg_response,0),1),
