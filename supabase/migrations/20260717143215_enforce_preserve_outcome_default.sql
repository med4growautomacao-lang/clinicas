-- 20260717143215_enforce_preserve_outcome_default
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- =============================================================================
-- Fase A0.2 — Inverte o default destrutivo do acoplamento etapa<->outcome
--
-- Antes: mover um ticket para etapa intermediária SEM mexer no outcome ZERAVA
-- o desfecho (outcome/outcome_at/loss_reason); preservar exigia a flag
-- app.keep_ticket_outcome='on'. Qualquer UPDATE cru (n8n Gatilhos, IA) apagava
-- venda em silêncio — 41 ocorrências medidas até 16/07.
--
-- Agora: PRESERVA por default; zerar exige intenção explícita via
-- app.reset_ticket_outcome='on'. Único consumidor legítimo: reopen_ticket
-- (botão "Cancelar" do Kanban). app.keep_ticket_outcome segue aceito por
-- compatibilidade (tornou-se redundante).
-- =============================================================================
...
BEGIN
  ...
  ELSIF v_stage_changed AND NOT v_outcome_changed THEN
    ...
    ELSIF v_slug = 'perdido' THEN
      NEW.outcome := 'perdido';
      NEW.outcome_at := COALESCE(NEW.outcome_at, now());
    ELSIF COALESCE(current_setting('app.reset_ticket_outcome', true), '') = 'on' THEN
      -- Zeramento EXPLÍCITO (reopen_ticket / "Cancelar" do Kanban)
      NEW.outcome := NULL;
      NEW.outcome_at := NULL;
      NEW.loss_reason := NULL;
    ELSE
      -- Default seguro: mover de etapa não apaga desfecho.
      NULL;
    END IF;
  END IF;
...
-- reopen_ticket é o único caminho que zera desfecho: declara a intenção.
...
BEGIN
  ...
  PERFORM set_config('app.reset_ticket_outcome', 'on', true);
  UPDATE tickets
    SET stage_id = p_new_stage_id, status = 'open', closed_at = NULL, loss_reason = NULL
  WHERE id = p_ticket_id;
  PERFORM set_config('app.reset_ticket_outcome', 'off', true);
