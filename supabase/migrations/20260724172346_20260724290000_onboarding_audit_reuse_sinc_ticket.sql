-- 20260724172346_20260724290000_onboarding_audit_reuse_sinc_ticket
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Resíduo do onboarding na origem: no caminho "paciente" só-passado, REUSA o ticket da Sincronização
-- como o próprio ticket ganho (em vez de inserir um novo e deixar o da Sincronização fechado para trás).
-- Elimina o ticket fechado órfão na etapa Sincronização sem precisar de exceção por slug no Kanban.
-- Mantém: #3 (lead_perdido liga IA) e #5 (COALESCE em todos os stage_id).
...
BEGIN
  ...
  IF p_last_appt_date IS NOT NULL AND p_next_appt_date IS NOT NULL THEN
    -- passado (ganho fechado, ticket novo) + próximo (reusa a Sincronização como agendado)
  ...
  ELSIF p_last_appt_date IS NOT NULL THEN
    -- só passado: REUSA a Sincronização como o próprio ticket ganho (sem deixar ticket fechado órfão)
