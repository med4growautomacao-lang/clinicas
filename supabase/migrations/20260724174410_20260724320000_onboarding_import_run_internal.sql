-- 20260724174410_20260724320000_onboarding_import_run_internal
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Refactor: as entranhas do import (pull do store uazapi -> leads/chat_messages) viram uma função
-- INTERNA sem trava (chamável pelo harvest do deep-sync, que roda sem auth.uid). O RPC público mantém
-- a trava fn_can_onboard + desliga follow-ups e delega para a interna. REVOKE da interna p/ anon/authenticated.
...
DECLARE
  ...
  v_lead uuid; v_ticket uuid; v_cut timestamptz := now() - interval '90 days';
...
REVOKE ALL ON FUNCTION public._onboarding_import_run(uuid) FROM anon, authenticated;

-- RPC público: trava + desliga follow-ups + delega.
...
BEGIN
  ...
  END IF;
  PERFORM fn_onboarding_disable_followups(p_clinic_id);  -- follow-ups off ao iniciar a organização
