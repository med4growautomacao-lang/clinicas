-- 20260713191338_lead_touchpoints_journey
...
create index if not exists idx_touchpoints_lead    on public.lead_touchpoints (lead_id, occurred_at) where lead_id is not null;
create index if not exists idx_touchpoints_rast    on public.lead_touchpoints (clinic_id, rast_id) where rast_id is not null;
create index if not exists idx_touchpoints_pending on public.lead_touchpoints (clinic_id, rast_id) where lead_id is null and rast_id is not null;
...
as $$
declare
  v_rast text; v_clinic uuid; v_n integer;
begin
  ...
  where t.clinic_id = v_clinic and t.rast_id = v_rast and t.lead_id is null;
