-- Duas correções no selector, ambas confirmadas em teste antes de escrever:
--
-- (A) CONTATO FORA DO WHATSAPP entrava no público. O Emissor bloqueia o envio (grava a linha
--     como 'dropped'), mas devolve id SEM erro, então a edge lia "enviado", gravava 'sent' em
--     automation_logs e o passo de despedida ainda chamava finalize_ticket: atendimento fechado
--     como Perdido sem a pessoa receber nada. É a mesma classe dos 12 casos de julho.
--     Hoje: 37 contatos marcados, 35 com atendimento aberto, 4 linhas de reengajamento já
--     'dropped'. A régua aqui ESPELHA a do emit_message (coalesce(whatsapp_invalid,false)).
--
-- (B) DESLIGAR O GATE TRAVAVA quem já estivesse numa régua de etapa. Com o gate off o selector
--     devolvia régua null enquanto o contato carregava X, e o claim exige casar contador E régua:
--     nenhuma das duas pernas fecha, o contato nunca mais é reservado e, como é re-selecionado a
--     cada 30 min com eligible_at no passado, ocupava a cota de 5 da clínica para sempre,
--     silenciando os outros. Testado: claim=false antes, claim=true depois.
--     Agora, com o gate off, o selector devolve a régua QUE O CONTATO JÁ TEM (preserva a marca) e
--     usa os passos do Padrão. O rollback volta a ser inócuo.
...
as $function$
  ...
  select
    ...
    -- (B) com o gate off, devolve a régua que o contato já carrega: é o que faz o claim casar
    ec.regua_gravar
  from gate g
  cross join leads l
  ...
  cross join lateral (
    select r.regua as regua_passos,
           case when g.ligado then r.regua else l.followup_ruleset_stage_id end as regua_gravar,
  ...
  join passos p
   ...
   and p.stage_id is not distinct from ec.regua_passos
   and p.pos = ec.expected + 1
  left join followup_rulesets cfg
    on cfg.clinic_id = l.clinic_id and cfg.stage_id is not distinct from ec.regua_passos
  where (p_clinic_id is null or l.clinic_id = p_clinic_id)
    ...
    -- (A) contato comprovadamente fora do WhatsApp não entra: o envio seria bloqueado pelo
    -- Emissor e a edge leria isso como sucesso. Mesma régua de emit_message.
...
comment on function public.fn_followup_candidates_reengagement(uuid) is
  'Candidatos ao reengajamento. Próximo passo por ORDEM dentro da régua; último passo ativo é a despedida quando a régua manda encerrar. Exclui contato fora do WhatsApp (espelha o gate do emit_message). Com o gate reengajamento_por_etapa desligado, os passos vêm do Padrão mas a régua gravada no contato é preservada, para o claim continuar casando.';
