-- 20260317211457_update_trigger_sla_breach_logic
...
BEGIN
    ...
    IF NEW.lead_id IS NOT NULL THEN
        IF NEW.sender = 'human' AND NEW.direction = 'outbound' THEN
            -- Atendente humano respondeu → pausa IA
        ...
        ELSIF NEW.direction = 'outbound' THEN
            -- Resposta outbound (IA ou sistema): verifica se estourou SLA antes de responder
            ...
            IF v_sla_minutes IS NOT NULL AND v_sla_minutes > 0 THEN
                ...
                -- Só conta breach se: lead mandou msg DEPOIS da última resposta (ciclo aberto)
                IF v_last_msg_at IS NOT NULL 
                   ...
                   AND (EXTRACT(EPOCH FROM (now() - v_last_msg_at)) / 60) > v_sla_minutes THEN
                    -- SLA estourou neste ciclo → incrementa +1
                    UPDATE public.leads 
        ...
        ELSIF NEW.sender = 'human' AND NEW.direction = 'inbound' THEN
            -- Lead mandou mensagem inbound → atualiza last_message_at
