-- ⚠️ ARQUIVO SUPERADO. NAO EDITE ESTE ARQUIVO PARA MEXER EM get_dashboard_stats_impl.
...
-- Visao Geral parava de carregar ("Nao foi possivel carregar os dados") nas clinicas grandes.
-- Causa: todo filtro de periodo era <coluna>::date BETWEEN p_date_from AND p_date_to. O cast
-- por linha impede o uso de indice, entao o painel lia o historico INTEIRO da clinica para
-- devolver um mes. Medido em 06/08/2026 (Metaltres, 61.175 mensagens): 11.711 ms contra o teto
-- de 8 s do role authenticated -> erro 57014 (statement timeout) -> faixa vermelha na tela.
-- Aqui a troca e por janela semiaberta [inicio, fim), que o indice consegue usar.
-- Numero nao muda: <col>::date BETWEEN a AND b e exatamente <col> >= a AND <col> < b+1.
...
DECLARE
  ...
  v_tzini timestamptz := ((p_date_from::timestamp) at time zone 'UTC');
  v_tzfim timestamptz := (((p_date_to + 1)::timestamp) at time zone 'UTC');
BEGIN
...
-- O grant vem por DOIS caminhos e revogar um so nao fecha nada (CLAUDE.md secao 1).
-- _impl nunca pode ser chamavel pelo PostgREST: quem tem o guard e o wrapper.
