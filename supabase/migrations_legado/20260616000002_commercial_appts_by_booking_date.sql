-- Painel Comercial: AGENDAMENTOS passam a contar pela DATA DA MARCAÇÃO
-- (appointments.created_at = quando a secretária/IA marcou na agenda), em vez de
-- appointments.date (data da consulta). Afeta, na janela de CONVERSÃO:
--   • Consultas Agendadas (total + IA/manual) e o detalhamento por status
--   • Consultas Realizadas (subconjunto do byStatus) e o denominador do Ticket Médio
--   • a série "Agendamentos" das Tendências
-- Mantém tudo no mesmo eixo para o total bater com o breakdown.
-- Resto da função idêntico a 20260613000026 (dois eixos Entrada/Conversão).
...
BEGIN
  ...
  -- consultas realizadas GERAL (denominador do ticket médio): por DATA DA MARCAÇÃO ∩ coorte ENTRADA
