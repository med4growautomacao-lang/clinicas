-- 20260720015137_followups_sender_system_and_confirm_ai_guard
...
-- 1) Gatilhos de etapa: 'system' NUNCA dispara regra (senão a msg de encerramento
--    "parabéns, seu contrato foi assinado" casaria com a regra de Ganho).
...
-- 2) Automação não é atendimento: system não atualiza last_outbound_at (não fecha ciclo de
--    SLA/1ª resposta). last_activity_at continua (ordenação de Conversas).
...
-- 3) SLA não-atendido: msg de system não conta como resposta (não silencia o alerta).
...
-- 4) Resposta de confirmação: vira 'system' + marca GUC transaction-local para o
--    ingest NÃO encaminhar a mensagem ao Agente IA (anti-dupla-resposta).
...
-- 7) Lembrete de confirmação: passa a REGISTRAR na conversa como 'system' (antes não aparecia).
...
-- 8) ingest_wa_message: NÃO encaminha à IA quando a resposta de confirmação foi tratada
--    nativamente (GUC setado pelo trigger na MESMA transação, durante o INSERT).
