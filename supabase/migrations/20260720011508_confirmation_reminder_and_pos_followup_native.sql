-- 20260720011508_confirmation_reminder_and_pos_followup_native
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Followup nativo: (A) lembrete de confirmação de consulta (substitui n8n "Envio de Confirmação")
-- e (B) pós-atendimento (nunca foi nativo). Ambos = cron varre candidatos + envia via
-- system_http_post + dedup. Espelham os gates do n8n (confirm_enabled, followup do lead, wa
-- conectado, expediente, janela de tempo).

-- Coluna de dedup do pós-atendimento (1 envio por ticket resolvido).
  ...
  loop
    begin
      ...
      if v_msg is null or btrim(v_msg) = '' then
        update tickets set pos_followup_sent_at = now() where id = r.ticket_id;  -- marca p/ não reprocessar
        continue;
      end if;
      if r.phone is null or r.api_token is null or btrim(r.api_token) = '' then continue; end if;
