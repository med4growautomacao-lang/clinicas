-- 20260719162532_report_settings_and_send
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Fase C dos relatórios: envio pelo WhatsApp da ORGANIZAÇÃO.
-- report_settings = config por clínica (destinatários + agenda p/ Fase D).
...
create table public.report_settings (
  ...
  send_weekday int not null default 1 check (send_weekday between 0 and 6),  -- 0=domingo … 1=segunda
  send_hour int not null default 8 check (send_hour between 0 and 23),      -- hora SP
  kind text not null default 'completo' check (kind in ('completo','geral','ia','humano')),
  period_days int not null default 7 check (period_days between 1 and 90),  -- janela: últimos N dias
  updated_at timestamptz not null default now()
);

alter table public.report_settings enable row level security;

-- org admins da org da clínica + admin da clínica gerenciam
...
declare
  ...
  v_sent int := 0;
begin
  -- Guard: mesmo do build (usuário logado precisa de vínculo)
