-- 20260721231111_get_commercial_leads_fix_gerados_distinct
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

CREATE OR REPLACE FUNCTION public.get_commercial_leads(p_clinic_id uuid, p_entry_from date, p_entry_to date, p_conv_from date, p_conv_to date, p_agent text DEFAULT 'todos'::text, p_origin text DEFAULT 'todos'::text, p_limit integer DEFAULT 20, p_offset integer DEFAULT 0, p_channel text DEFAULT 'todos'::text, p_metric text DEFAULT 'todos'::text, p_agenda_from date DEFAULT NULL::date, p_agenda_to date DEFAULT NULL::date, p_sort text DEFAULT 'entrada'::text, p_sort_dir text DEFAULT 'desc'::text, p_outcome text DEFAULT 'ambos'::text, p_loss_reasons text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
