-- 20260729030137_conv_ai_mine_candidates_fn
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- =============================================================================
-- Minerador de candidatos do motor mecânico (Passo 1).
-- Dada uma clínica, extrai n-gramas (2/3/4 palavras) das mensagens de um lado
-- (saída por padrão) entre leads RESOLVIDOS, e mede o lift de cada gram contra a
-- taxa-base de ganho da clínica. Devolve só candidatos (NÃO grava). A curadoria
-- por LLM (próximo passo) é que decide o alvo (etapa) e descarta o ruído.
--
-- Só funciona onde há gabarito real (leads com outcome ganho/perdido). Clínica de
-- funil abandonado (sem outcome) precisa da via de PAR auto-evidente, não desta.
--
-- SECURITY DEFINER porque agrega sobre chat_messages (RLS by-passada de propósito
-- para a agregação). EXECUTE fechado: só service_role (o backend/minerador). Nunca
-- anon/authenticated — este repo vigia função interna aberta.
-- =============================================================================
...
grams as (
  ...
  from msgs m
  cross join (values (2),(3),(4)) as k(n)
  cross join generate_subscripts(m.w,1) as i
  where array_length(m.w,1) >= k.n and i + k.n - 1 <= array_length(m.w,1)
),
lg as (
