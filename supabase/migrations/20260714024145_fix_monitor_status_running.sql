-- 20260714024145_fix_monitor_status_running
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- FALSO POSITIVO (pego no teste): o monitor acusava a si mesmo.
-- O pg_cron usa vários status transitórios ('starting', 'running', 'sending'…) além de
-- 'succeeded' e 'failed'. Eu tratava "tudo que não é succeeded" como falha — então, ao ler
-- cron.job_run_details, o próprio monitor via a SUA execução em curso marcada como 'running' e
-- registrava um erro. Só 'failed' é falha.
