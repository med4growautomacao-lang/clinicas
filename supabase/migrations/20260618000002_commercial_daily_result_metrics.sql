-- Painel Comercial — série DIÁRIA com métricas de RESULTADO para o gráfico de Tendências.
-- Acrescenta ao daily: realizadas (consultas por a.date), ganhos (vendas por outcome_at),
-- faturamento (receita paga por ft.date) e faturamentoProjetado (agend. ativos criados × ticket).
-- Mantém o resto igual à 20260618000001 (status/attended por a.date).
...
BEGIN
  ...
  SELECT jsonb_agg(jsonb_build_object(
    ...
    'realizadas', COALESCE(rz.realizadas, 0), 'ganhos', COALESCE(wg.ganhos, 0),
    'faturamento', COALESCE(rev.faturamento, 0),
    'faturamentoProjetado', COALESCE(a.valid_appts, 0) * COALESCE(v_default_ticket, 0),
  ...
  LEFT JOIN wg ON wg.d = dates.d LEFT JOIN hd h ON h.d = dates.d LEFT JOIN fu f ON f.d = dates.d;
