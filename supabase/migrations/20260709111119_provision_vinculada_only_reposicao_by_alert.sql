-- 20260709111119_provision_vinculada_only_reposicao_by_alert
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Correção de regra: na APROVAÇÃO só gera OP VINCULADA (o déficit do pedido — cliente esperando).
-- A REPOSIÇÃO (repor até o estoque mínimo) NÃO é automática: vira ALERTA (precisa_reposicao na
-- view) que o operador vê e programa manualmente (Nova OP no módulo Produção). Assim o chão de
-- fábrica não é inundado de OP de reposição a cada venda.
...
BEGIN
  ...
  LOOP
    ...
    -- SOB MEDIDA: produz a quantidade exata, sem reservar
    ...
    -- OP VINCULADA só do que o PEDIDO não cobre (déficit = pedido − disponível − produção livre).
    -- Reposição até o mínimo NÃO é automática — sai como alerta (view precisa_reposicao).
    v_deficit := v_comp - (v_disp + v_livre);
    IF v_deficit > 0 THEN
      IF COALESCE(v_item.lote_minimo, 0) > 0 THEN
        v_qtdop := ceil(v_deficit / v_item.lote_minimo) * v_item.lote_minimo;   -- respeita o lote de produção
      ELSE
        v_qtdop := v_deficit;
      END IF;
      INSERT INTO public.production_orders
...
-- View: disponível + reposição em andamento + FLAG de estoque mínimo (o alerta que o operador vê).
