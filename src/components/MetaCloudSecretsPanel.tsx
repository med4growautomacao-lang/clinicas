...
// Painel Super Admin › Configurações › "API Meta" — credenciais GLOBAIS da WhatsApp Cloud API
// oficial (token + WABA), usadas pela edge meta-cloud-api para criar/enviar templates no plano
// Meta Tester.
//
// MESMO padrão do Google Ads OAuth: guardadas CRIPTOGRAFADAS no Vault; nunca exibidas de volta
// (só o status). Escrita via set_meta_cloud_secret (super-admin); a edge lê via
// get_meta_cloud_secret (service_role). NÃO usar system_settings — SELECT é público e vazaria o
// token, que dá acesso de envio ao WhatsApp de todas as clínicas do plano.

type Field = "token" | "waba_id";

const FIELD_META: Record<Field, { label: string; hint: string }> = {
  token:   { label: "Token de Acesso", hint: "System User token com whatsapp_business_messaging + whatsapp_business_management" },
  waba_id: { label: "WABA ID",         hint: "WhatsApp Business Account ID (ex.: 882390987724948)" },
};
const FIELDS: Field[] = ["token", "waba_id"];

export function MetaCloudSecretsPanel() {
  const [status, setStatus] = useState<Record<Field, boolean>>({ token: false, waba_id: false });
  const [loading, setLoading] = useState(true);
  const [input, setInput] = useState<Record<Field, string>>({ token: "", waba_id: "" });
  const [saving, setSaving] = useState<Field | null>(null);
  const [msg, setMsg] = useState<{ kind: "ok" | "err"; text: string } | null>(null);

  const load = async () => {
    setLoading(true);
    const { data: st } = await supabase.rpc("meta_cloud_secrets_status");
  ...
  const saveField = async (field: Field) => {
    ...
    setSaving(field);
    const { error } = await supabase.rpc("set_meta_cloud_secret", { p_key: field, p_value: value });
    setSaving(null);
  ...
  const removeField = async (field: Field) => {
    setSaving(field);
    const { error } = await supabase.rpc("delete_meta_cloud_secret", { p_key: field });
    setSaving(null);
  ...
  if (loading) {
    return (
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-10 flex items-center justify-center gap-3 text-slate-400">
        <Loader2 className="w-5 h-5 animate-spin" /> <span className="text-sm font-medium">Carregando credenciais da API Meta…</span>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-xs text-slate-500 mt-1">
          Token e WABA usados pela edge <b>meta-cloud-api</b> para criar e enviar templates no plano <b>Meta Tester</b>.
          {allSet
            ? <span className="text-emerald-600 font-semibold"> Tudo configurado — criação e envio de templates já podem rodar.</span>
            : <span className="text-amber-600 font-semibold"> Enquanto os dois não estiverem preenchidos, criar/enviar template retorna "não configurado".</span>}
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
        <div>
          <h3 className="text-sm font-black text-slate-900 flex items-center gap-2"><KeyRound className="w-4 h-4 text-slate-400" /> Credenciais da Cloud API</h3>
            ...
            <div key={field} className="rounded-xl border border-slate-200 bg-slate-50/50 p-3 flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="bg-teal-600 hover:bg-teal-700 disabled:opacity-40 text-white px-3 py-2 rounded-xl font-bold flex items-center gap-1.5 text-sm transition-colors"
                >
