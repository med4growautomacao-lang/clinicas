-- Decisao do dono (03/08/2026): por enquanto o aviso de queda vai SO pelo sino e
-- pela faixa no topo do app (WhatsAppStatusBanner, que ja existia e ja cobre o caso:
-- le whatsapp_instances.status por Realtime, some para medico e para quem nao
-- contratou o canal, e tem botao "Reconectar" que abre a tela certa).
-- A mensagem no grupo fica para depois.
--
-- ⚠️ MUDANCA DE SEMANTICA, e ela e o ponto desta migration: o canal GRUPO deste
-- evento passa de opt-OUT (`?? true`) para opt-IN (`?? false`). Tudo o mais que ja
-- estava construido continua de pe (preferencia do numero da ORG, link de reconexao,
-- antirrepeticao de 3h, req_id auditavel): basta ligar a chave na tela para voltar.
--
-- ⚠️ A TELA MUDA JUNTO, na mesma leva (NotificationPrefs.tsx ganhou `grupoPadrao`).
-- Deixar o backend em `false` e a tela mostrando o toggle LIGADO seria a mesma classe
-- de defeito que este trabalho todo vem evitando: tela dizendo uma coisa, sistema
-- fazendo outra (§0.7).
--
-- O SINO NAO muda: segue opt-out (`?? true`), porque junto da faixa e o unico canal
-- que funciona mesmo com o WhatsApp fora do ar.
--
-- Preservado da versao anterior (20260804023842), para quando o grupo voltar:
...
-- dois entregaria a mensagem DUAS VEZES no alarme falso.
...
-- notificacao, senao o envio falha calado. Por isso o request_id do pg_net e gravado
-- no evento: da para conferir o desfecho depois em net._http_response.
...
begin
  ...
  -- Sino: opt-OUT (nasce ligado). Direto em notifications, sem passar por notify_ops,
  -- que aplicaria `sino_all` e suprimiria 27 das 28 clinicas. Junto da faixa no topo
  -- do app, e o que funciona SEMPRE, porque nao depende de WhatsApp nenhum.
  ...
  -- Grupo: opt-IN (nasce DESLIGADO). Ligar so na tela, por clinica.
  v_pode := coalesce((v_ev->>'grupo')::boolean, false);
  if not v_pode then
    return jsonb_build_object('enviado', false, 'motivo', 'grupo_desligado', 'sino', v_sino);
  end if;
