-- 20260719034548_forms_welcome_message_gates
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Regras extras do welcome de forms:
--  (1) só dispara se o lead NÃO tem nenhuma mensagem (não recebeu nem enviou) — se já tem conversa,
--      não dá boas-vindas. Substitui o gate anterior por last_message_at (mais preciso).
...
BEGIN
    FOR r IN
        ...
        WHERE l.capture_channel = 'forms'
          ...
          -- (1) lead sem NENHUMA mensagem (não recebeu nem enviou)
          AND NOT EXISTS (SELECT 1 FROM public.chat_messages cm WHERE cm.lead_id = l.id)
          -- (2) lead novo: no máximo 3 dias desde a criação
