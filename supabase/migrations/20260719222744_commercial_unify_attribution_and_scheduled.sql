-- 20260719222744_commercial_unify_attribution_and_scheduled
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Fase 4 da unificação: get_commercial_dashboard passa a usar as MESMAS fontes
-- canônicas da Visão Geral para atribuição IA×Humano e agendamentos gerados.
--   (a) leadsTouched: cálculo inline (maioria de mensagens, varria chat_messages
--       ao vivo) -> vw_lead_agent_class (precompute lead_kpi_attribution).
...
-- Método seguro: replace ancorado no def atual + asserts; não transcreve os 35k.
DO $mig$
