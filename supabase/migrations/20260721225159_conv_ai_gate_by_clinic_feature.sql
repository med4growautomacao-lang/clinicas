-- 20260721225159_conv_ai_gate_by_clinic_feature
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- A funcionalidade "Sugestões da IA" agora é liberada por clínica no Super Admin
-- (clinics.features.feature_conv_ai). O gate tem que ser REAL: sem ele, uma
-- clínica com a aba escondida continuaria enfileirando e pagando análise de LLM
-- sem ninguém ver o resultado.
--
-- Opt-in: só vale com === true. Duas portas para fechar: o trigger que enfileira
-- e o claim que entrega o lote para a edge.
...
BEGIN
  ...
  -- Early-exit barato: precisa da funcionalidade liberada E da análise ligada.
  IF NOT EXISTS (
    SELECT 1
...
-- A canário já estava rodando antes da flag existir: liga a funcionalidade nela
-- para não interromper o teste em andamento.
UPDATE public.clinics
   SET features = COALESCE(features, '{}'::jsonb) || '{"feature_conv_ai": true}'::jsonb
 WHERE id = '2c9c4e85-df66-41f6-b345-8b7ec94f0605';
