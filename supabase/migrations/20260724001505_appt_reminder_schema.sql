-- 20260724001505_appt_reminder_schema
...
alter table public.ai_config
  ...
  add column if not exists appt_reminder_lead_time        integer not null default 120,
  add column if not exists appt_reminder_window_start     integer not null default 8,
  add column if not exists appt_reminder_window_end       integer not null default 20,
  add column if not exists appt_reminder_grace_minutes    integer not null default 60,
  add column if not exists appt_reminder_only_confirmed   boolean not null default false;
