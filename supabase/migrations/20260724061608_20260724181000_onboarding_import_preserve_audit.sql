-- 20260724061608_20260724181000_onboarding_import_preserve_audit
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Corrige idempotência import x auditoria: re-importar NÃO pode ressuscitar na Sincronização
-- um lead já auditado. Só cria ticket na Sincronização para lead SEM nenhum ticket (novo).
...
BEGIN
  ...
  LOOP
    ...
    -- Ticket: reusa o aberto; se não há aberto mas o lead JÁ TEM ticket (auditado), anexa ao mais recente
    -- (NÃO ressuscita na Sincronização); só cria na Sincronização se o lead não tem ticket nenhum.
