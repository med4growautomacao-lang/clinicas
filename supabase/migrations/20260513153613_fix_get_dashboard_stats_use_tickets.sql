-- 20260513153613_fix_get_dashboard_stats_use_tickets
...
BEGIN
  ...
  -- VENDAS: tickets que entraram em "ganho" no período (fonte = tickets.stage_id, não leads.stage_id deprecado)
  SELECT COUNT(DISTINCT t.lead_id) INTO v_total_sales FROM tickets t
  WHERE t.clinic_id = p_clinic_id AND t.stage_id = v_ganho_stage_id
  ...
  -- Ciclo de vendas via lead_stage_history (alimentado pelo trigger em tickets)
  ...
  sales_d AS (SELECT COALESCE(t.outcome_at, t.closed_at)::date AS date, COUNT(DISTINCT t.lead_id) as qty
    FROM tickets t
    WHERE t.clinic_id = p_clinic_id AND t.stage_id = v_ganho_stage_id
      AND COALESCE(t.outcome_at, t.closed_at)::date BETWEEN p_date_from AND p_date_to
