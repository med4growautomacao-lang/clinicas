-- 20260718204734_get_dashboard_stats_v2_canonical
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- get_dashboard_stats v2: unifica com a fonte da verdade dos outros painéis.
-- MUDANÇAS: (a) totalAppointments = v_kpi_scheduled (união agendamento∪etapa, dedupe,
-- por data de virou-agendado); (b) faturamento = valor lançado (conversions EXCLUINDO
-- 'Orçamento Enviado') no card (nova key salesValue) e no gráfico, no lugar do financeiro;
-- totalRevenue (financeiro) mantido no JSON 1 release p/ compat/rollback do front.
-- Atribuição já vem certa/rápida via vw_lead_agent_class (aponta p/ lead_kpi_attribution).
...
BEGIN
  ...
  WHERE clinic_id = p_clinic_id AND date BETWEEN p_date_from AND p_date_to
    AND (p_origin = 'todos'
      OR (CASE WHEN platform = 'meta_ads' THEN 'meta' WHEN platform = 'google_ads' THEN 'google' WHEN platform = 'balcao' THEN 'balcao' ELSE 'sem_origem' END) = ANY(string_to_array(p_origin, ',')));
