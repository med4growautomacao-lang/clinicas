-- 20260708225435_scheduling_buffer_and_hardening
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Correções no agendamento (auditoria 08/07): buffer fix + min_notice reschedule + advisory lock + revoke anon
...
-- (2)+(3) book_appointment — advisory lock por (doctor_id, date)
...
-- (2)+(3) reschedule_appointment — min_notice via requester_phone + advisory lock
...
-- (4) Segurança: revoga anon/PUBLIC das RPCs de escrita (mantém authenticated + service_role)
