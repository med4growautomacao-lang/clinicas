-- 20260618062638_revenue_scoped_anchored_on_realized_appointments
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- revenueScoped agora = receita das CONSULTAS REALIZADAS no recorte, ancorado nas mesmas
-- consultas que a métrica "Consultas Realizadas": parte de appointments (status realizado/
-- compareceu), soma a receita ligada (appointment_id), e usa o MESMO lead (ticket->lead)
-- para origem/coorte. Assim não conta venda avulsa sem consulta (ex.: ganho sem appointment)
-- nem usa lead diferente do paciente. Conv window pela data da consulta (ap.date).
DO $do$
DECLARE src text;
BEGIN
