-- 20260403020721_ensure_whatsapp_leads_upsert_logic_is_clean
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Remove a trigger de criação se ela estiver conflitando ou ajusta a função mestre
-- para garantir que o insert use a cláusula ON CONFLICT
...
BEGIN
    ...
    IF NEW.clinic_id IS NOT NULL AND (NEW.lead_id IS NULL OR NEW.phone IS NULL) THEN
        ...
        IF v_lead_phone IS NOT NULL AND v_lead_phone <> '' AND v_lead_phone <> COALESCE(v_clinic_phone, '') THEN
            -- BUSCA O ID PARA EVITAR DUPLICIDADE ANTES DE TENTAR CRIAR
            ...
            IF v_ref_lead_id IS NULL THEN
                ...
                -- USANDO ON CONFLICT EXPLICITAMENTE PARA EVITAR O ERRO 409
                ...
                ON CONFLICT (clinic_id, phone) DO UPDATE SET updated_at = now()
    ...
    -- Lógica de automação segue normal
    ...
    IF NEW.clinic_id IS NOT NULL AND NEW.lead_id IS NOT NULL AND json_data IS NOT NULL THEN
        ...
        IF v_msg_content IS NOT NULL THEN
            IF v_handoff_rules IS NOT NULL THEN
                FOR v_rule IN SELECT * FROM jsonb_array_elements(v_handoff_rules) LOOP
                    FOR v_keyword IN SELECT trim(val) FROM unnest(string_to_array(v_rule->>'keywords', ',')) val LOOP
