-- 20260720024549_followup_staleness_guards
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Travas contra followups vencidos (cenário: followup desativado e religado dias depois).
-- Trava 1 (cap de vencimento) + Trava 2 (rate-limit no pós) + Trava 3 (expiração observável).
...
-- Retirada do acúmulo histórico (one-shot, silencioso).
...
-- PÓS-ATENDIMENTO: cap + rate-limit + expiração observável
...
-- REENGAJAMENTO: cap de idade da última mensagem
