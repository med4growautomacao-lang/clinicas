-- Janela de envio configurável para o lembrete de confirmação (só a 1ª mensagem "antes da
-- consulta"; a RESPOSTA não tem janela — responde na hora que o paciente clica). Igual
-- welcome/reengajamento. Default 6-22 (mantém o comportamento anterior fixo).
alter table public.ai_config add column if not exists confirm_window_start integer not null default 6;
alter table public.ai_config add column if not exists confirm_window_end   integer not null default 22;
