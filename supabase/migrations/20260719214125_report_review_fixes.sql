-- 20260719214125_report_review_fixes
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Correções da code review (achados #8, #10, #12).

-- #8 e #10 em build_commercial_report (replace cirúrgico no def atual):
--   (a) cabeçalho não pode virar NULL se um par de datas vier meio-aberto
--       (to_char(NULL) anulava toda a concatenação).
...
-- #12 em run_scheduled_reports: advisory lock por transação evita sobreposição de dois ticks do cron.
DO $mig3$
DECLARE v_def text; v_new text;
BEGIN
  SELECT pg_get_functiondef(p.oid) INTO v_def
