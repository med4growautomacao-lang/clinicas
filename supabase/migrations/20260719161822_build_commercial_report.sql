-- 20260719161822_build_commercial_report
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Motor ÚNICO do relatório comercial (texto p/ WhatsApp).
-- Fonte única: o botão da tela E o envio agendado (Fase D) chamam ESTA função —
-- a montagem client-side do ComercialDashboard.tsx é substituída por ela.
-- Os números vêm de get_commercial_dashboard (mesmos da tela, sempre batem) +
-- blocos novos: Marketing (v_kpi_investment), Perdas (tickets/loss_reason) e
-- comparativo ▲▼ vs. período anterior equivalente.
...
-- Auxiliar: delta percentual "(▲ 12%)" / "(▼ 8%)"; null se sem base de comparação
...
begin
  ...
  -- Comparativo: cada janela fornecida deslocada pelo próprio tamanho (período anterior equivalente)
  ...
  if p_kind in ('completo','geral') then
    ...
    lines := lines || ('*💰 FINANCEIRO*');
    lines := lines || ('• Vendas lançadas: *' ||
      case when g_vendas_lancadas > 0 then _report_brl(g_vendas_lancadas) else '—' end || '*'
      || coalesce(' ' || d_vendas, ''));
    lines := lines || ('• Previsto (agendados × ticket): *' ||
      case when g_fat_prev is not null and g_fat_prev > 0 then _report_brl(g_fat_prev) else '—' end || '*');
    if g_invest > 0 then
      lines := lines || ('• Investido em anúncios: *' || _report_brl(g_invest) || '*' || coalesce(' ' || d_inv, ''));
      if g_vendas_lancadas > 0 then
        lines := lines || ('• Retorno realizado: *R$ ' ||
      ...
      if g_fat_prev is not null and g_fat_prev > 0 then
        lines := lines || ('• Retorno previsto: *R$ ' ||
    ...
    v_blocks := v_blocks || array_to_string(lines, E'\n');

    -- ===== Bloco MARKETING (investimento por plataforma + CPL/CAC) =====
    if g_invest > 0 then
      ...
      if m_meta > 0 or m_google > 0 then
        lines := lines || ('• Investimento: Meta *' || _report_brl(m_meta) || '* · Google *' || _report_brl(m_google) || '*');
      end if;
      if m_cpl is not null then
        lines := lines || ('• Custo por interessado (CPL): *' || _report_brl(m_cpl) || '*');
      end if;
      if m_cac is not null then
        lines := lines || ('• Custo por cliente (CAC): *' || _report_brl(m_cac) || '*');
      end if;
      v_blocks := v_blocks || array_to_string(lines, E'\n');
    end if;

    -- ===== Bloco PERDAS (motivos + no-show) =====
    if g_lost > 0 or g_noshow > 0 then
      ...
      if p_top is not null then
        lines := lines || ('• Principais motivos: ' || p_top);
      end if;
      if p_noshow_base > 0 then
        lines := lines || ('• Faltas (no-show): *' || g_noshow || ' de ' || p_noshow_base || '* ('
  ...
  -- ===== Blocos por AGENTE (IA / Humano) — mesmos números da tela (scoped) =====
  ...
  foreach a_is_ia in array (case
    ...
    if a_is_ia then
      lines := lines || ('• Resolveu sozinha (sem humano): *' || coalesce((a->'agents'->'ia'->>'autonomous')::int, 0) || '*');
      lines := lines || ('• Passou p/ humano: *' || coalesce((a->'agents'->'ia'->>'handoffs')::int, 0) || '*');
    else
      lines := lines || ('• Assumiu da IA: *' || coalesce((a->'agents'->'humano'->>'handoffsReceived')::int, 0) || '* conversas');
    end if;
    if a_fatprev is not null and a_fatprev > 0 then
      lines := lines || ('• Previsto (agendados × ticket): *' || _report_brl(a_fatprev) || '*');
    end if;
    v_blocks := v_blocks || array_to_string(lines, E'\n');
  end loop;
