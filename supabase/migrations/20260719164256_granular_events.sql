-- 20260719164256_granular_events
...
as $$
declare
  v_event text; v_title text; v_level text;
  v_patient text; v_doctor text; v_lead uuid; v_when text;
begin
...
begin
  ...
  if TG_OP = 'UPDATE' and OLD.outcome is not distinct from 'ganho' then return NEW; end if;
  if NEW.lead_id is null then return NEW; end if;
