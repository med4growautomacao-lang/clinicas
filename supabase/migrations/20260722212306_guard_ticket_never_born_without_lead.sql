-- 20260722212306_guard_ticket_never_born_without_lead
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Duas brechas que deixam nascer ticket sem lead. Nenhuma era a origem dos órfãos de 22/07
-- (essa era o cron delete_pending_leads), mas ambas produzem o mesmo estrago silencioso.
...
BEGIN
  ...
  -- TRAVA: sem lead não se abre ticket. Ticket órfão some do Kanban e não entra em painel
  -- nenhum (v_kpi_wins faz INNER JOIN com leads) — falha silenciosa clássica.
  IF v_lead_id IS NULL THEN
...
-- (2) fn_auto_move_lead_to_agendado: mesma falha do set_ticket_stage já corrigido — ao abrir
-- "novo ciclo" copiava v_ticket.lead_id do ticket de origem. Ticket órfão gerava outro órfão.
