-- Selector nativo do reengajamento (substitui o Schedule+SELECT do n8n). Espelha
-- process_forms_followup: percorre os elegíveis e net.http_post por lead → edge reengagement-followup.
-- Gate por fatos DURÁVEIS (converted_patient_id / ganho / is_not_lead) + ai_enabled + handoff +
-- etapa não-terminal. Inatividade por INBOUND e comparação 100% em horário de São Paulo.
...
-- Cron a cada 15 min (a janela 6-22 SP é checada dentro da função).
do $cleanup$
begin
  perform cron.unschedule(jobid) from cron.job where jobname = 'reengagement_followup_job';
exception when others then null;
end $cleanup$;

select cron.schedule(
  'reengagement_followup_job',
  '*/15 * * * *',
  $$ select public.process_reengagement_followup(); $$
);
