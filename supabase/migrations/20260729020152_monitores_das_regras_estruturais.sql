-- 20260729020152_monitores_das_regras_estruturais
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Monitores das regras estruturais do CLAUDE.md na Central de Erros.
--
-- Quatro regras que hoje so existem escritas viram vigilancia automatica, no mesmo cron de 5 min
-- (system_monitors_job) e no mesmo estilo dos monitores que ja existem: log_system_error(...,
-- p_is_monitor => true), fingerprint md5('monitor|<code>|-') empilhado em v_tocados, e auto-resolve
-- no fim (o que parou de acender sai do painel sozinho, via trg_system_error_arquiva_resolvido).
--
--   (a) grant_indevido_<funcao>   funcao interna (_impl e backend-only) com EXECUTE para anon/authenticated
--   (b) http_sem_rastro_<funcao>  chamada HTTP crua no lugar de system_http_post
--   (c) invariante_sumiu_<indice> um dos 4 indices que seguram as invariantes sumiu/ficou invalido
--   (d) painel_sem_guard_<rpc>    wrapper de painel sem assert_clinic_access (ou sem wrapper nenhum)
--
-- Estado medido no banco vivo antes de aplicar: os quatro blocos devolvem ZERO linha.
-- A migration nao acende nada no painel; ela so passa a acender quando alguem regredir.

-- TRAVA DE SESSAO CONCORRENTE. `create or replace` troca o corpo INTEIRO: um monitor que outra
-- sessao tenha acrescentado entre a revisao e a aplicacao seria apagado SEM ERRO NENHUM.
do $guarda$
declare
  v_src text;
  v_md5 text;
begin
  select p.prosrc into v_src
    from pg_proc p
   where p.proname = 'run_system_monitors'
     and p.pronamespace = 'public'::regnamespace;

  if v_src is null then
    raise exception 'ABORTADO: public.run_system_monitors() nao existe neste banco.';
  end if;

  if position('grant_indevido_' in v_src) > 0 then
    raise notice 'run_system_monitors ja contem os monitores estruturais; reaplicando a mesma versao.';
    return;
  end if;

  v_md5 := md5(v_src);
  if v_md5 <> '239818f90f07b1de37634462da500a7f' then
    raise exception 'ABORTADO: o corpo de run_system_monitors mudou desde a revisao (md5 atual: %, esperado: %). Outra sessao provavelmente acrescentou um monitor, e aplicar agora apagaria esse monitor em silencio.',
      v_md5, '239818f90f07b1de37634462da500a7f';
  end if;
end
$guarda$;
...
-- Cinto de seguranca: a regra da casa e sempre revogar dos DOIS caminhos, PUBLIC e nominal.
-- O cron roda como postgres, dono da funcao, entao nao depende de grant nenhum.
