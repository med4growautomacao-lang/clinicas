-- Correções do code-review do Lembrete de Consulta (9 findings). As funções do Lembrete são novas e
-- estão DESLIGADAS em todas as clínicas, então isto não muda comportamento em produção — mas conserta
-- bugs reais antes do go-live. A exclusão no reengajamento (#4) é a única que toca função compartilhada.

-- ============================================================ #7 fonte única da fórmula de vencimento
-- eligible/expires estavam escritos no candidato E reescritos no UPDATE de vencimento do motor.
-- Um helper imutável elimina a divergência silenciosa.
create or replace function public.fn_appt_reminder_expires(
  p_appt timestamp, p_lead_min int, p_grace_min int
) returns timestamp language sql immutable set search_path to 'public' as $$
  select least(
    p_appt - (coalesce(p_lead_min, 120) || ' minutes')::interval
           + (coalesce(p_grace_min, 60) || ' minutes')::interval,
    p_appt
  )::timestamp
$$;

comment on function public.fn_appt_reminder_expires(timestamp,int,int) is
  'Vencimento do lembrete: eligible (appt - lead) + grace, nunca depois da própria consulta. Fonte única usada pelo candidato e pelo corte de vencimento do motor.';

-- ============================================================ #9 candidato: excluir is_not_lead
-- (o pós-atendimento já exclui; o lembrete não excluía). Assinatura inalterada → CREATE OR REPLACE.
...
-- ============================================================ #1 + #2 + #7 motor
--   #1 dedup_key inclui a data/hora ALVO: remarcar gera chave nova (antes, a chave só-appointment_id
--      colidia no índice único do Emissor e o lembrete do horário novo era descartado em silêncio).
--   #2 caminho do Emissor NÃO é mais barrado por wa_ok: enfileira e deixa o worker reentregar. O
--      pré-filtro wa_ok anulava o retry justamente numa mensagem que vence em minutos. O caminho
--      inline segue exigindo token (fn_clinic_send_token → continue se nulo).
--   #7 vencimento via fn_appt_reminder_expires (mesma fórmula do candidato).
  ...
  loop
    begin
      ...
      if public.fn_emissor_ativo(r.clinic_id) then
        -- dedup_key inclui a data/hora alvo: remarca gera chave nova; o worker resolve token e reentrega.
        ...
        -- Caminho antigo (chave desligada): sem retry, então exige WhatsApp apto na hora.
...
-- ============================================================ #8 índice de apoio ao cron de 1/min
-- O candidato varre appointments filtrando "ainda não enviado/vencido". Índice parcial evita seq scan.
create index if not exists ix_appt_reminder_pending
  on public.appointments (clinic_id, date)
  where appt_reminder_sent_at is null and appt_reminder_expired_at is null;

-- ============================================================ #4 reengajamento não pega quem tem consulta marcada
-- O lembrete grava outbound sender='system' e vira a última mensagem; em clínica com funil customizado
-- (sem o slug 'agendado') isso poderia rearmar o reengajamento. Em vez de depender do slug, exclui pelo
-- FATO: lead com consulta futura pendente/confirmada nunca é alvo de reengajamento. (Só ADICIONA um gate
-- de exclusão; nenhuma outra regra do seletor muda.)
...
as $function$
  ...
  where (p_clinic_id is null or l.clinic_id = p_clinic_id)
    ...
    -- NOVO (#4): quem tem consulta futura marcada não é reengajado (independe do slug do funil).
