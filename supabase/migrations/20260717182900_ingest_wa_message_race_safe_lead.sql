-- 20260717182900_ingest_wa_message_race_safe_lead
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Fix: criação de lead race-safe. Em rajada de mensagens de um lead NOVO, duas
-- execuções paralelas passam pelo SELECT (nada) e ambas tentam INSERT → colisão
-- em leads_clinic_id_phone_key. Agora o INSERT trata unique_violation re-buscando
-- o lead (a corrida vira no-op idempotente, não perda de mensagem).
...
BEGIN
  ...
  IF v_lead.id IS NULL AND p_direction = 'inbound' THEN
    ...
    EXCEPTION WHEN unique_violation THEN
      -- corrida: outra execução criou o lead entre o SELECT e o INSERT
      SELECT id, ai_enabled, is_not_lead INTO v_lead
      FROM leads
      WHERE clinic_id = v_clinic AND (phone = v_norm OR normalize_br_phone(phone) = v_norm)
      ORDER BY last_activity_at DESC NULLS LAST
      LIMIT 1;
    END;
  END IF;
