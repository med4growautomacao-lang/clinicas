-- 20260613143525_create_get_commercial_dashboard_rpc
...
DECLARE
  ...
  v_auto jsonb;
  v_sla_breaches int; v_pending int; v_avg_first_response numeric; v_sla_minutes int;
...
BEGIN
  ...
  -- pending response: snapshot (clinic-wide, lead aguardando resposta)
  ...
  -- avg first response (min), para leads criados no periodo
  SELECT COALESCE(AVG(EXTRACT(EPOCH FROM (fo.first_out - nl.created_at)) / 60.0), 0)
  ...
  JOIN (
    ...
    FROM chat_messages
    WHERE clinic_id = p_clinic_id AND direction = 'outbound'
      AND created_at::date BETWEEN p_date_from AND p_date_to
    GROUP BY lead_id
  ...
  -- ===== OUTCOMES (tickets ganho/perdido) =====
  SELECT
  ...
  -- ===== FUNNEL (por ticket, ultima entrada, no periodo) =====
  WITH entries AS (
...
GRANT EXECUTE ON FUNCTION public.get_commercial_dashboard(uuid, date, date) TO authenticated;
