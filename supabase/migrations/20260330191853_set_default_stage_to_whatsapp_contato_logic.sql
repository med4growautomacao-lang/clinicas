-- 20260330191853_set_default_stage_to_whatsapp_contato_logic
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- 1. Atualizar a função que define a etapa padrão para novos Leads
...
BEGIN
    IF NEW.stage_id IS NULL THEN
        -- Tenta encontrar especificamente a etapa "Contato via WhatsApp"
        ...
        -- Se não encontrar, tenta qualquer etapa que tenha "whatsapp" ou "contato" no nome
        ...
        -- Se ainda não encontrar, pega a de posição inicial
        IF v_stage_id IS NULL THEN
            SELECT id INTO v_stage_id 
            FROM public.funnel_stages 
            WHERE clinic_id = NEW.clinic_id 
...
-- 2. Atualizar a função principal do gatilho de mensagens para garantir que novos leads criados lá também sigam essa regra
...
BEGIN
    ...
    -- [C] CAPTURA/CRIAÇÃO DE LEAD (FORÇANDO ETAPA CONTATO VIA WHATSAPP)
    IF NEW.clinic_id IS NOT NULL AND (NEW.lead_id IS NULL OR NEW.phone IS NULL) THEN
        ...
        IF v_lead_phone IS NOT NULL AND v_lead_phone <> '' AND v_lead_phone <> COALESCE(v_clinic_phone, '') THEN
            ...
            IF v_ref_lead_id IS NULL THEN
                -- PRIORIDADE PARA "CONTATO VIA WHATSAPP"
                SELECT id INTO v_stage_id 
                FROM public.funnel_stages 
                WHERE clinic_id = NEW.clinic_id 
                  AND LOWER(name) ILIKE '%contato%whatsapp%'
                ...
                -- Fallback se não encontrar a específica
                IF v_stage_id IS NULL THEN
                ...
                ON CONFLICT (clinic_id, phone) DO UPDATE SET updated_at = EXCLUDED.updated_at
    ...
    IF NEW.clinic_id IS NOT NULL AND NEW.lead_id IS NOT NULL AND json_data IS NOT NULL THEN
                    ...
                    LOOP
                        IF v_keyword <> '' AND v_msg_content ILIKE '%' || v_keyword || '%' THEN
                            IF v_rule->>'target_stage_id' IS NOT NULL AND v_rule->>'target_stage_id' <> '' THEN
                                UPDATE public.leads SET stage_id = (v_rule->>'target_stage_id')::uuid, updated_at = now() WHERE id = NEW.lead_id;
                            END IF;
                            EXIT; 
                        END IF;
                    END LOOP;
        ...
        IF NOT v_is_handoff AND NEW.sender = 'ai' THEN
            IF v_msg_content ILIKE '%transferir%atendente%' OR v_msg_content ILIKE '%gatilho:%' OR v_msg_content ILIKE '%handoff%' THEN
                v_is_handoff := true;
            END IF;
        END IF;
    END IF;
    ...
    IF NEW.lead_id IS NOT NULL THEN
        ...
        ELSIF NEW.direction = 'outbound' THEN
            IF v_is_handoff THEN
                UPDATE public.leads SET ai_enabled = false, updated_at = now() WHERE id = NEW.lead_id;
            ELSE
                SELECT sla_minutes, business_hours INTO v_sla_minutes, v_business_hours FROM public.ai_config WHERE clinic_id = NEW.clinic_id;
                IF v_sla_minutes IS NOT NULL AND v_sla_minutes > 0 THEN
                    SELECT last_message_at, last_outbound_at INTO v_last_msg_at, v_last_out_at FROM public.leads WHERE id = NEW.lead_id;
                    IF v_last_msg_at IS NOT NULL AND (v_last_out_at IS NULL OR v_last_msg_at > v_last_out_at) AND (EXTRACT(EPOCH FROM (now() - v_last_msg_at)) / 60) > v_sla_minutes THEN
                        UPDATE public.leads SET sla_breach_count = sla_breach_count + 1, last_outbound_at = now(), updated_at = now() WHERE id = NEW.lead_id;
                    ELSE
