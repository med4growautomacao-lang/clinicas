...
--   • firstResponseMin = MÉDIA do 1º ciclo de cada lead, TEMPO CORRIDO  ("Tempo da 1ª resposta")
--   • responseMin      = MÉDIA de TODOS os ciclos, TEMPO CORRIDO         ("Tempo de resposta")
--   • breaches         = nº de ciclos acima da meta, MINUTOS ÚTEIS       ("Estouros de SLA")
--   • overBreachMin    = MÉDIA de (ciclo − meta) nos estouros, ÚTEIS     ("Tempo estourado")
--   • responseCycles   = nº total de ciclos (p/ distinguir "instantâneo" de "sem respostas")
-- Velocidade em tempo corrido (média, captura sub-minuto); SLA/estouro em minutos úteis.
-- TETO de 1h SÓ para respostas da IA: descarta ciclos da IA com gap > 60min (reengajamentos
-- pareados com inbound antigo, que não são resposta real). Humano não tem teto.
...
BEGIN
  ...
  cyc AS (
    ...
    WHERE kind = 'out' AND prev_kind = 'in'
      -- teto de 1h só p/ IA: descarta reengajamentos (gap absurdo) que não são resposta real
  ...
  SELECT
    ...
    COALESCE((SELECT AVG(biz - v_sla_minutes) FROM cyc WHERE v_sla_minutes > 0 AND biz > v_sla_minutes), 0),
