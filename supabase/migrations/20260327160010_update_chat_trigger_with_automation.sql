-- 20260327160010_update_chat_trigger_with_automation
            ...
            LOOP
                ...
                LOOP
                    IF v_keyword <> '' AND v_msg_content ILIKE '%' || v_keyword || '%' THEN
                        -- 1. Aplica mudança de etapa se definida
                        IF v_rule->>'move_to_stage' IS NOT NULL AND v_rule->>'move_to_stage' <> '' THEN
                            UPDATE public.leads 
                            SET stage_id = (v_rule->>'move_to_stage')::uuid,
                                updated_at = now()
                            WHERE id = NEW.lead_id;
                        END IF;
                        
                        -- 2. Aplica ação de transbordo se configurado
                        ...
                        EXIT; -- Sai do loop de keywords
                    END IF;
                END LOOP;
            END LOOP;
        END IF;
        
        -- Fallback para detecção legada se não bateu em nenhuma regra específica
