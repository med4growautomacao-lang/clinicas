-- Mensagem de abertura: um texto normal apresentando a empresa, enviado UMA vez, antes da
-- primeira pergunta.
--
-- Por que separar: a saudação vivia dentro do balão do menu, então a primeira coisa que o contato
-- via já era um formulário. Separando, ele recebe uma mensagem de gente e só depois o menu com a
-- foto do produto e os botões.
--
-- ⚠️ Mora em COLUNA do roteiro, e não dentro da definição versionada, de propósito: ela só importa
-- no instante em que a conversa começa, então não há o que "pinar" numa versão. Trocar o texto vale
-- para quem começar a conversar dali em diante, e não mexe em ninguém no meio do roteiro.

alter table public.chatbot_scripts
  add column if not exists mensagem_abertura text;

comment on column public.chatbot_scripts.mensagem_abertura is
  'Texto simples enviado uma vez, antes da primeira pergunta. Vazio = o roteiro começa direto na primeira pergunta.';

-- `fn_chatbot_apresentar` ganha atraso. Drop antes: assinatura diferente criaria uma SEGUNDA
-- função (sobrecarga), e as chamadas por nome de parâmetro passariam a dar "function is not unique".
drop function if exists public.fn_chatbot_apresentar(uuid, uuid, uuid, text, jsonb, text, text, int);
...
begin
  ...
  -- Foto em mensagem separada SÓ no caminho de texto: em menu, ou é botão (a foto vai dentro, no
  -- imageButton) ou é lista (e foto é proibida na publicação).
  if v_midia is not null and (v_r->>'kind') <> 'menu' then
    perform public.emit_message(
      ...
      p_lead_id => p_lead_id, p_dedup_key => v_dedup || ':foto', p_delay_ms => p_delay_ms);
  end if;
  ...
  if v_r->>'kind' = 'menu' then
    v_out := public.emit_message(
      ...
      p_body => v_body, p_lead_id => p_lead_id, p_dedup_key => v_dedup,
      p_chat_payload => v_payload, p_delay_ms => p_delay_ms);
  else
    v_out := public.emit_message(
      p_clinic_id => p_clinic_id, p_to_addr => p_phone, p_producer => 'chatbot',
      p_kind => 'text', p_body => v_body, p_lead_id => p_lead_id,
      p_dedup_key => v_dedup, p_chat_payload => v_payload, p_delay_ms => p_delay_ms);
  end if;
...
-- O turno: manda a abertura antes da primeira pergunta.
    ...
    if v_nova and v_abertura is not null then
      perform public.emit_message(
        ...
        p_chat_payload => jsonb_build_object('sender','system','message', jsonb_build_object(
          ...
          'additional_kwargs','{}'::jsonb,'response_metadata','{}'::jsonb)));
      -- A pergunta sai logo atrás na mesma fila (que é ordenada por conversa), com um respiro de
      -- "digitando" para as duas não caírem no mesmo segundo.
      v_atraso := 1500;
...
revoke all on function public.fn_chatbot_apresentar(uuid, uuid, uuid, text, jsonb, text, text, int, int) from public, anon, authenticated;
