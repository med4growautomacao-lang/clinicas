...
--   • avgFirstResponseMin     = MÉDIA do 1º ciclo de cada lead     ("Tempo médio da 1ª resposta")
--   • avgResponseMin          = média de TODOS os ciclos            ("Tempo médio de resposta")
--   • breaches                = nº de ciclos acima da meta          ("Estouros de SLA")
--   • avgOverBreachMin        = média de (ciclo − meta) nos estouros ("Tempo médio estourado")
...
BEGIN
  ...
  SELECT
    COALESCE((SELECT AVG(biz) FROM firsts), 0),
    COALESCE((SELECT AVG(biz) FROM cyc), 0),
  ...
  RETURN jsonb_build_object(
    ...
    'sla', jsonb_build_object(
      'avgFirstResponseMin', ROUND(COALESCE(v_median_first_response,0),1),
      'avgResponseMin', ROUND(COALESCE(v_avg_response,0),1),
      'breaches', COALESCE(v_sla_breaches,0),
      'avgOverBreachMin', ROUND(COALESCE(v_avg_over_breach,0),1),
