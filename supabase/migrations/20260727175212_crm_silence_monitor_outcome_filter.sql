-- O monitor de silencio do CRM parava de enxergar (achado #1 do review de 27/07).
-- Par da migration 20260727175211_crm_lead_intake_fixes.sql.
--
-- crm_desfecho_silencioso decide "a integracao morreu?" por max(received_at) de external_crm_events.
-- Sem filtrar outcome, bastava o CRM seguir mandando LEAD para o monitor achar que estava tudo vivo
-- enquanto ganho/perdido morria em silencio: exatamente o incidente de 18/07 que criou este monitor.
...
begin
  ...
  -- de DESFECHO há 3+ dias (inclui "nunca recebeu"). Sinal de que a automação do CRM parou.
  -- ⚠️ O filtro outcome in ('ganho','perdido') e load-bearing: sem ele, um CRM que so manda
  -- ?tipo=lead mantem o max(received_at) fresco e o monitor nunca acende, mesmo com o desfecho morto.
  for r in
