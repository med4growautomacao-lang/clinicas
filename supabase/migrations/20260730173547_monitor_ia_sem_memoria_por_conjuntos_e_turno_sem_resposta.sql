-- Dois consertos nos monitores, achados na revisao adversarial de 30/07/2026:
--   (e) ia_sem_memoria comparava min(session_id) dos dois lados e ficava MUDO no split parcial
--   (g) monitor NOVO: turno do paciente que a IA nunca respondeu (perda no claim da fila)
-- e um comentario honesto sobre o alcance real do bloco (f).
--
-- Gerado por cirurgia de string sobre a definicao viva (scratchpad/patch_monitores.js) para nao
-- arriscar transcrever 30 mil caracteres a mao. O restante da funcao e byte a byte o que estava
-- em producao.
  ...
  begin
    for r in
      with ultima_entrada as (
        select cm.clinic_id, cm.lead_id, max(cm.seq) as seq_ult, max(cm.created_at) as quando
          from chat_messages cm
