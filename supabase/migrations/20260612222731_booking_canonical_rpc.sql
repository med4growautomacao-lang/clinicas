...
CREATE FUNCTION public.convert_lead_to_appointment(
  p_clinic_id uuid, p_lead_id uuid, p_doctor_id uuid, p_date date, p_time time without time zone,
  p_modality text DEFAULT 'presencial', p_notes text DEFAULT NULL, p_ticket_id uuid DEFAULT NULL,
  p_duration_minutes integer DEFAULT NULL, p_request_id uuid DEFAULT NULL, p_consultation_type_id uuid DEFAULT NULL)
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
BEGIN
  RETURN public.book_appointment(p_clinic_id=>p_clinic_id, p_doctor_id=>p_doctor_id, p_date=>p_date, p_time=>p_time,
    p_patient_name=>NULL, p_patient_phone=>NULL, p_duration_minutes=>p_duration_minutes, p_source=>'manual',
    p_modality=>p_modality, p_notes=>p_notes, p_request_id=>p_request_id, p_consultation_type_id=>p_consultation_type_id, p_lead_id=>p_lead_id);
END; $function$;
