-- 20260317180616_fix_lead_phone_extraction_logic
...
BEGIN
    ...
    IF NEW.clinic_id IS NOT NULL AND (NEW.lead_id IS NULL OR NEW.phone IS NULL) THEN
        ...
        -- 1. Prioriza NEW.phone se já vier preenchido
        v_lead_phone := NEW.phone;

        -- 2. Se phone não foi fornecido, tenta extrair do session_id
        IF (v_lead_phone IS NULL OR v_lead_phone = '') AND NEW.session_id IS NOT NULL THEN
            ...
            ELSIF NEW.session_id <> COALESCE(v_clinic_phone, '') THEN
                -- session_id não começa com clinic phone e não É o clinic phone → é o lead phone
                v_lead_phone := NEW.session_id;
            END IF;
            -- Se session_id = clinic_phone exatamente, v_lead_phone fica NULL (skip)
        END IF;

        -- 3. Cria/associa lead apenas se temos um telefone válido e diferente da clínica
