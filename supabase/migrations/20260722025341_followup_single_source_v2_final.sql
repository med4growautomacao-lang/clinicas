-- 20260722025341_followup_single_source_v2_final
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Definição FINAL e consolidada da fonte única dos follow-ups (substitui integralmente o que as
-- migrations 20260721000020 e 20260721000021 descreviam; ambas viraram no-op no repo).
--
-- Correções desta versão, vindas do review:
--  1) O join direto com whatsapp_instances nas funções de candidatos filtrava a LINHA por status
--     antes e passou a não filtrar nada, o que duplicaria a linha em clínica com 2 instâncias e
--     poderia mandar o token da instância errada. O join SAIU: token e aptidão viram função por
--     clínica (fn_clinic_send_token), impossibilitando duplicação por construção.
--  2) api_token deixou de ser devolvido ao preview (ele nunca usou; segredo não trafega à toa).
--  3) wa_ok passa a ser avaliado UMA VEZ POR CLÍNICA (view v_clinic_send_state) em vez de por linha.
--  4) Corte de tempo unificado em <= nos quatro motores, alinhado ao balde "agora" do preview.
--  5) process_forms_followup e process_reengagement_followup ganham registro na Central de Erros.
...
-- confirmação e pós perdem a coluna api_token: precisa de drop+create na MESMA transação.
...
exception when others then
  -- Sem isto o motor parava e ninguém ficava sabendo (o rastro ia só p/ cron.job_run_details).
