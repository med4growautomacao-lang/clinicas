-- 20260729195332_20260724410000_onboarding_include_unclassified_funnel_leads
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Tipo C: leads que JÁ ESTAVAM no funil, com conversa e ainda em aberto, que nunca foram
-- classificados (não têm consulta na agenda nem venda, então não caem no Tipo B, e não têm ticket
-- na Sincronização, então não caem no Tipo A). Na Lorena eram 125 parados em "Contato via
-- WhatsApp"/Qualificado/etc. Ficam de FORA: resolvidos (perdido/ganho, outcome preenchido),
-- is_not_lead, já revisados e quem não tem conversa nenhuma.
...
-- Preserva ANOTAÇÃO HUMANA: o reset da auditoria só limpa nota gerada pelo próprio onboarding;
-- e a nota de agendamento passa a ser ACRESCENTADA, não a substituir o que a equipe escreveu.
