-- 20260709085603_provision_keep_orcamento_id_on_reposicao
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Melhoria: OP de reposição também guarda orcamento_id (rastreabilidade da venda que disparou o
-- restock + idempotência via índice orcamento_id+line_key). A alocação (vinculada×reposição) é
-- definida pelo `tipo`, não pelo vínculo — a produção livre já filtra por tipo='reposicao'.
