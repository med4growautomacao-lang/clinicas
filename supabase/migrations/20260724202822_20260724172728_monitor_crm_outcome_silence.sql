-- 20260724202822_20260724172728_monitor_crm_outcome_silence
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Adiciona monitor "crm_desfecho_silencioso" ao run_system_monitors: avisa na Central de Erros
-- quando uma clínica com desfecho do CRM externo habilitado (crm_token + won/lost) segue captando
-- lead (operante), mas parou de receber external_crm_events há 3+ dias. É o caso da Intubação
-- (leads entram pela automação do cliente, mas ganho/perdido do Clint parou). Segue o padrão dos
-- demais monitores: empilha fingerprint em v_tocados e is_monitor=true (auto-resolve quando voltar).
...
begin
  ...
  -- Desfecho do CRM externo silencioso: clínica com integração de ganho/perdido habilitada
  -- (crm_token + won/lost) e leads entrando nos últimos 3 dias, mas SEM external_crm_events
  -- há 3+ dias (inclui "nunca recebeu"). Sinal de que a automação do CRM parou de nos chamar.
  for r in
    select ci.clinic_id, c.name,
           (select max(e.received_at) from external_crm_events e where e.clinic_id = ci.clinic_id) as ultimo_evento
    ...
    where ci.crm_token is not null
      ...
      and coalesce(
        (select max(e.received_at) from external_crm_events e where e.clinic_id = ci.clinic_id),
        'epoch'::timestamptz
      ) < now() - interval '3 days'
  loop
    ...
    perform public.log_system_error(
      ...
      jsonb_build_object(
        ...
        'obs', 'Cliente com desfecho habilitado e leads ativos, mas external_crm_events parou. Conferir a automacao do CRM (ex.: Clint) apontando para external-crm-status?k=<crm_token>&tipo=ganho|perdido.'
      ),
      true
    );
    n_mon := n_mon + 1;
