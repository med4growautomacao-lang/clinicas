-- 20260613193628_commercial_dashboard_realized_by_consultation_date
...
BEGIN
  ...
  -- byStatus por DATA DA CONSULTA (a.date): realizadas/no-show contam quando a consulta acontece (misto)
  ...
  SELECT
    COUNT(DISTINCT t.lead_id) FILTER (WHERE t.outcome = 'ganho'),
    COUNT(DISTINCT t.lead_id) FILTER (WHERE t.outcome = 'perdido')
  INTO v_won, v_lost
  FROM tickets t
