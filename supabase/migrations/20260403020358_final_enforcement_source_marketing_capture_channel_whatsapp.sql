-- 20260403020358_final_enforcement_source_marketing_capture_channel_whatsapp
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- 1. Restaura a regra rígida (Agora que os dados estão limpos, vai funcionar!)
ALTER TABLE leads DROP CONSTRAINT IF EXISTS leads_source_check;
ALTER TABLE leads ADD CONSTRAINT leads_source_check 
CHECK (source = ANY (ARRAY['meta_ads'::text, 'google_ads'::text]));

-- 2. Atualiza a função definitiva para salvar no lugar certo
    ...
    IF NEW.clinic_id IS NOT NULL AND (NEW.lead_id IS NULL OR NEW.phone IS NULL) THEN
        ...
        IF v_lead_phone IS NOT NULL AND v_lead_phone <> '' AND v_lead_phone <> COALESCE(v_clinic_phone, '') THEN
            ...
            IF v_ref_lead_id IS NULL THEN
                ...
                -- SALVANDO CORRETAMENTE EM CAPTURE_CHANNEL
                INSERT INTO public.leads (clinic_id, name, phone, source, capture_channel, stage_id)
                VALUES (NEW.clinic_id, 'Lead ' || v_lead_phone, v_lead_phone, NULL, 'whatsapp', v_stage_id)
