-- 20260717172500_ingest_wa_message
...
--
-- UMA viagem ao banco faz tudo: resolve a clínica pelo api_token da instância,
-- resolve/cria o lead por telefone NORMALIZADO (dos dois lados), insere a
-- mensagem (dedup automático via wa_message_id/C1) e devolve os gates de IA
-- para o edge decidir o forward. Quem insere dispara os triggers canônicos
-- (auto_open_ticket, gatilhos de etapa, session_id, seq, etc.).
-- =============================================================================
...
BEGIN
  ...
  IF v_msg_id IS NULL THEN
    v_duplicate := true;  -- dedup por wa_message_id (tr_chat_message_aa_dedup)
  END IF;
  ...
  v_forward := p_direction = 'inbound'
    ...
    AND (COALESCE(v_cfg.test_mode_enabled, false) = false
         OR position(v_norm IN COALESCE(v_cfg.test_numbers, '')) > 0);
