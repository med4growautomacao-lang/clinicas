-- Quem zera o desfecho tem que zerar TAMBÉM as colunas novas.
--
-- Sem isto, `reopen_ticket` e o zeramento explícito da trigger limpariam só o texto e deixariam
-- `loss_reason_slug` preenchido — justamente a coluna que os painéis vão passar a usar como fonte.
-- Seria a mesma sujeira de hoje (13 tickets 'ganho' com motivo de perda), recriada na coluna nova.

-- ------------------------------------------------------------------ reopen_ticket
-- Corpo idêntico ao anterior; muda SÓ o UPDATE final (2 colunas a mais).
...
revoke all on function public.reopen_ticket(uuid, uuid, boolean) from public, anon, authenticated;
grant execute on function public.reopen_ticket(uuid, uuid, boolean) to authenticated, service_role;

-- ------------------------------------------------------------------ trigger de consistência
-- Corpo idêntico; muda SÓ o ramo do zeramento explícito (app.reset_ticket_outcome='on').
...
BEGIN
  ...
  ELSIF v_stage_changed AND NOT v_outcome_changed THEN
    ...
    ELSIF COALESCE(current_setting('app.reset_ticket_outcome', true), '') = 'on' THEN
      ...
      NEW.loss_reason_slug := NULL;   -- <- novo
      NEW.loss_note := NULL;          -- <- novo
    ELSE
