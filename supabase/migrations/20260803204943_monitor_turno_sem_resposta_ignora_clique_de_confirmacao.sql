-- Tira o falso positivo do monitor turno_sem_resposta (criado horas antes, no mesmo dia).
--
-- Clique nos botoes do lembrete de confirmacao ("Confirmar consulta", "Remarcar", "Cancelar") nao
-- deve ser respondido pela IA: fn_handle_confirmation_reply levanta app.confirmation_handled e o
-- ingest_wa_message desliga o forward_ai de proposito, porque quem trata o clique e o gatilho, que
-- marca a consulta. A resposta pos-confirmacao so sai se a clinica ligar confirm_post_enabled.
--
-- Sem esse recorte o monitor acusava atendimento PERFEITO como falha: medido em 02/08/2026 na
-- clinica Tyago Venancio, a paciente clicou "Confirmar consulta" as 20:00, a consulta foi para
-- status='confirmado' e o alerta subiu como critical e ficou aberto ~23h. Alarme falso recorrente
-- treina o dono a ignorar o painel, e um painel ignorado nao vigia nada.
--
-- De passagem, a CTE virou DISTINCT ON para trazer o TEXTO da ultima mensagem (o max(seq) sozinho
-- nao dava acesso ao conteudo). O casamento e identico ao do gatilho, para os dois nao discordarem.
--
-- Gerado por cirurgia de string sobre a definicao viva; o resto da funcao e byte a byte producao.
...
begin
  ...
  begin
    for r in
      -- CONJUNTOS, nao min(). Com min() o monitor calava no split PARCIAL: se a IA gravasse 1x na
      -- sessao certa e Nx numa errada que ordena DEPOIS, min(ai) = min(inbound) e nada acendia.
      -- Os 2 casos reais de split que restaram na base (09/07 e 02/07) sao exatamente dessa forma
      -- (cauda com telefone cru, que ordena depois da normalizada). Agora acende se QUALQUER sessao
      -- em que a IA escreveu estiver fora do conjunto de sessoes em que o paciente escreveu.
      with recentes as (
        select cm.clinic_id, cm.lead_id,
               array_agg(distinct cm.session_id) filter (where cm.sender = 'ai') as sess_ai,
               array_agg(distinct cm.session_id) filter (where cm.direction = 'inbound') as sess_in
      ...
      select x.clinic_id, c.name, count(*) as leads
       ...
       where x.sess_ai is not null and x.sess_in is not null
         and exists (select 1 from unnest(x.sess_ai) s where not (s = any (x.sess_in)))
  ...
  -- ⚠️ Este bloco e a rede SECUNDARIA, e a janela de 24h e por created_at, que na linha importada e
  -- a data ORIGINAL da mensagem. Ou seja: duplicata de mensagem antiga NAO entra aqui. O alerta
  -- primario mora onde a duplicata nasce, no _onboarding_import_run ('import_em_clinica_viva'),
  -- que nao pode ficar cego porque dispara no momento do insert. Trocar o eixo para seq aqui foi
  -- medido e custa 1,35s com duas varreduras completas de chat_messages: caro para um cron de 5min,
  -- e por isso a responsabilidade ficou no import.
  ...
  -- (g) Turno do paciente que a IA NUNCA respondeu.
  -- claim_due_ai_turns faz DELETE ... RETURNING: a partir do claim o turno so existe na memoria do
  -- worker. Se o isolate morrer no meio (deploy, teto de CPU/wall-clock, OOM), o paciente fica sem
  -- resposta, a linha do buffer JA foi apagada, o sweep do cron nao tem o que varrer e nem o
  -- registrarErro roda (morreu com o isolate). Era perda silenciosa perfeita: a mensagem do
  -- paciente fica na conversa e ninguem sabe que ficou sem resposta.
  -- Recorte de proposito estreito para nao virar alarme de negocio: clinica com IA ligada, lead com
  -- IA ligada, sem cadeado, e que JA foi atendido pela IA antes (prova que o caminho funciona para
  -- ele). Assim o alerta significa "a IA parou de responder no meio", nao "esse contato nunca teve
  -- resposta". Medido em 30/07: zero em 2 dias, entra mudo.
  begin
    for r in
      with ultima_entrada as (
        select distinct on (cm.clinic_id, cm.lead_id)
               cm.clinic_id, cm.lead_id, cm.seq as seq_ult, cm.created_at as quando,
               lower(btrim(coalesce(cm.message->>'content',''))) as texto
          from chat_messages cm
         where cm.direction = 'inbound'
           and cm.lead_id is not null
           and cm.created_at >= (now() at time zone 'America/Sao_Paulo') - interval '2 days'
           and cm.created_at <= (now() at time zone 'America/Sao_Paulo') - interval '15 minutes'
         order by cm.clinic_id, cm.lead_id, cm.seq desc
      )
      select u.clinic_id, c.name, count(*) as leads, min(u.quando) as mais_antigo
        from ultima_entrada u
        join clinics c on c.id = u.clinic_id
        join leads l on l.id = u.lead_id
        join ai_config ac on ac.clinic_id = u.clinic_id
       where coalesce(ac.auto_schedule, false)
         and l.ai_enabled is true
         and coalesce(l.is_not_lead, false) = false
         and coalesce(l.human_only, false) = false
         and coalesce(l.is_simulation, false) = false
         and exists (select 1 from chat_messages a where a.lead_id = u.lead_id and a.sender = 'ai')
         -- ⚠️ CLIQUE NO BOTAO DE CONFIRMACAO NAO E TURNO PERDIDO. fn_handle_confirmation_reply
         -- levanta app.confirmation_handled e o ingest DESLIGA o forward_ai de proposito: quem
         -- trata o clique e o gatilho (marca a consulta), nao o agente. E a resposta pos-confirmacao
         -- so sai se a clinica ligar confirm_post_enabled na tela, que nasce desligado. Sem este
         -- recorte o monitor acusava atendimento perfeito: medido em 02/08, paciente Miriam clicou
         -- "Confirmar consulta", a consulta foi para status='confirmado' e o alerta subiu como
         -- critical. Alarme falso recorrente treina o dono a ignorar o painel (CLAUDE.md).
         -- Casamento IDENTICO ao do gatilho, para os dois nunca discordarem.
         and not (u.texto like '%confirmar consulta%' or u.texto = 'confirmado'
               or u.texto like '%remarcar consulta%' or u.texto in ('remarcar','remarcado')
               or u.texto like '%cancelar consulta%' or u.texto = 'cancelado')
         and not exists (select 1 from chat_messages o
                          where o.lead_id = u.lead_id and o.direction = 'outbound'
                            and o.seq > u.seq_ult)
       group by u.clinic_id, c.name
    loop
      v_tocados := v_tocados || md5('monitor|turno_sem_resposta|' || r.clinic_id::text);
      perform public.log_system_error(
        'monitor', 'turno_sem_resposta',
        r.leads || ' paciente(s) falaram e a IA nao respondeu, em ' || r.name,
        'critical', r.clinic_id,
        jsonb_build_object(
          'leads', r.leads, 'mais_antigo', r.mais_antigo,
          'obs', 'A IA ja atendia esses leads e parou de responder no meio. O turno e APAGADO da fila '
                 || 'no claim (claim_due_ai_turns), entao worker que morre no meio perde o turno sem '
                 || 'deixar rastro. Conferir system_errors de turno_quebrou/resposta_vazia/'
                 || 'enfileirar_falhou na mesma janela e outbound_messages presas em pending.'),
        true
      );
      n_mon := n_mon + 1;
    end loop;
  exception when others then
    v_tocados := v_tocados
      || md5('monitor|monitor_falhou_turno_sem_resposta|-')
      || coalesce((select array_agg(e.fingerprint) from public.system_errors e
                    where e.is_monitor and e.status <> 'resolved'
                      and e.code = 'turno_sem_resposta'), array[]::text[]);
    perform public.log_system_error(
      'monitor', 'monitor_falhou_turno_sem_resposta',
      'A vigilancia de paciente sem resposta da IA parou de rodar (esse risco ficou sem vigia)',
      'error', null, jsonb_build_object('erro', SQLERRM, 'sqlstate', SQLSTATE), true
    );
    n_falhas := n_falhas + 1;
  end;
