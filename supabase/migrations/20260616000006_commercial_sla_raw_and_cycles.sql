...
--   • firstResponseMin = MEDIANA do 1º ciclo de cada lead, TEMPO CORRIDO ("Tempo da 1ª resposta")
--   • responseMin      = MEDIANA de TODOS os ciclos, TEMPO CORRIDO        ("Tempo de resposta")
--   • breaches         = nº de ciclos acima da meta, MINUTOS ÚTEIS        ("Estouros de SLA")
--   • overBreachMin    = MEDIANA de (ciclo − meta) nos estouros, ÚTEIS    ("Tempo estourado")
--   • responseCycles   = nº total de ciclos (p/ distinguir "instantâneo" de "sem respostas")
-- Velocidade em tempo corrido (mediana, captura sub-minuto); SLA/estouro em minutos úteis.
...
BEGIN
  ...
  SELECT
    COALESCE((SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY raw_min) FROM firsts), 0),
    COALESCE((SELECT percentile_cont(0.5) WITHIN GROUP (ORDER BY raw_min) FROM cyc), 0),
  ...
  INTO v_median_first_response, v_avg_response, v_sla_breaches, v_avg_over_breach, v_response_cycles;
