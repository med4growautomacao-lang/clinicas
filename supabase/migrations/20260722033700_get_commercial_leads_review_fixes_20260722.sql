-- 20260722033700_get_commercial_leads_review_fixes_20260722
      ...
      AND (
        ...
        OR (p_metric IN ('gerados', 'realizadas', 'marcados') AND EXISTS (
          ...
          WHERE t.lead_id = l.id AND a.clinic_id = p_clinic_id
            ...
            AND (p_agenda_from IS NULL OR a.created_at::date >= p_agenda_from)
            AND (p_agenda_to   IS NULL OR a.created_at::date <= p_agenda_to)
            -- Mesma régua de get_commercial_dashboard.v_appt_generated/byStatus
            -- (fn_lead_matches_agent, não a.source) — clicar no card "Agendamentos
            -- Gerados" com Agente ativo tinha que bater com esta lista, e não
            -- batia (achado de code-review 21/07).
