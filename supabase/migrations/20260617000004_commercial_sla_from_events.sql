...
--   • firstResponseMin/responseMin = MÉDIA dos ciclos em TEMPO CORRIDO (velocidade); teto 1h IA
--   • responseCycles               = nº de ciclos (p/ distinguir "instantâneo" de "sem respostas")
--   • breaches / overBreachMin     = vêm da tabela de EVENTOS sla_breaches (fonte única):
--       count + média do overshoot, por breached_at ∩ coorte entrada ∩ origem ∩ agente (sender),
--       com teto de 1h só p/ IA (NOT (sender='ai' AND wait_raw_min>60)).
-- Velocidade em tempo corrido (média); estouros em minutos úteis vindos do event-log.
