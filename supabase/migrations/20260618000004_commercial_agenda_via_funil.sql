-- Painel Comercial — AGENDA VIA FUNIL (clinics.features.agenda_via_funil = true).
-- Para clínicas que NÃO usam a agenda (appointments) e marcam etapas no funil por gatilho/Kanban:
--   Agendamentos      = entradas (distinct ticket) na etapa slug 'agendado'      (lead_stage_history)
--   Consultas Realiz. = entradas na etapa slug 'ganho'                            (gatilho de realizado)
--   Faltas            = entradas na etapa slug 'faltou_cancelou'
-- Ancorado em changed_at. Demais clínicas seguem pela tabela appointments (inalterado).
...
BEGIN
  ...
  -- ===== OVERRIDE: AGENDA VIA FUNIL (agendamentos/realizadas/faltas pelas etapas) =====
  IF v_agenda_funil THEN
    -- Agendamentos = entradas (distinct ticket) na etapa 'agendado'; ignora filtro de agente.
    ...
    -- Realizadas = entradas em 'ganho'; Faltas = entradas em 'faltou_cancelou'.
    SELECT
