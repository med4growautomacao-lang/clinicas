-- 20260512033247_phase2_finalize_appointment_link
...
BEGIN
  ...
  IF v_lead_id IS NOT NULL AND p_value > 0 THEN
    ...
    ON CONFLICT (lead_id, financial_transaction_id) DO NOTHING
    RETURNING id INTO v_conv_id;
  END IF;

  IF p_ticket_id IS NOT NULL THEN
    SELECT id INTO v_ganho_stage_id FROM funnel_stages
    WHERE clinic_id = v_apt.clinic_id AND slug = 'ganho' LIMIT 1;
    IF v_ganho_stage_id IS NOT NULL THEN
      UPDATE tickets SET stage_id = v_ganho_stage_id, outcome = 'ganho', outcome_at = now()
