-- 20260718225902_marketing_utm_funnel_cohort_ordered
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Adiciona ORDER BY determinístico ao marketing_utm_funnel_cohort para que a
-- paginação por .range() no front seja estável (a resposta REST é clampada em
-- max_rows=5000; este coorte passa disso em clínicas grandes e cortava o Ganho
-- em silêncio). Sem ORDER BY, offsets podem sobrepor/pular linhas. Corpo idêntico
-- ao anterior, só com a ordenação no final.
...
AS $function$
  ...
  ORDER BY (last_entry)::date, stage_id, platform, channel,
           utm_source, utm_campaign, utm_adset, utm_ad, utm_term, loss_reason;
$function$;
