-- FONTE ÚNICA do predicado dos follow-ups (definição CANÔNICA).
-- Substitui integralmente a 20260721000020 (virou no-op); a 021 ficou só com o ALTER TABLE.
--
-- PROBLEMA ORIGINAL: o preview de ativação carregava sua PRÓPRIA cópia dos gates de cada motor.
-- Enquanto for cópia, editar um selector faz a janela MENTIR em silêncio (não quebra, só passa a
-- contar errado) e é justamente a veracidade dela que dá valor à trava.
--
-- DIVISÃO DE RESPONSABILIDADE:
--   fn_followup_candidates_* = gates duráveis + eligible_at (quando o lead passa a poder receber)
--                              + toggle_on / wa_ok / janela, expostos como COLUNAS.
...
--
-- DECISÕES QUE VIERAM DOS REVIEWS (não desfazer sem entender o porquê):
--   • "WhatsApp apto a enviar" é UM conceito: fn_clinic_send_token. Já foram quatro definições
--     diferentes espalhadas, e a do pós não checava api_token, o que travava a fila: linha sem
--     token era escolhida, caía no `continue` sem marcar nada e ocupava o slot do cap toda rodada.
--   • As funções NÃO fazem join com whatsapp_instances. O join direto duplicaria a linha em clínica
--     com 2 instâncias (mensagem repetida ao paciente) e podia devolver o token de uma instância
--     diferente da que passou no gate. O motor pede o token por clínica na hora de enviar.
--   • api_token NÃO é devolvido: o preview nunca precisou dele e é SECURITY DEFINER exposto ao app.
--   • wa_ok é avaliado UMA VEZ POR CLÍNICA (view v_clinic_send_state), não por linha candidata.
--   • Corte de tempo é <= nos quatro motores, alinhado ao balde "agora" do preview.
--
-- EQUIVALÊNCIA VERIFICADA a cada cutover, comparando conjunto antigo x novo por clínica:
--   welcome 77=77 · reengajamento 973=973 · confirmação 16=16 · pós 725=725, diferença ZERO.

-- ============================================================ conceito único de envio
create or replace function public.fn_clinic_send_token(p_clinic_id uuid)
...
comment on function public.fn_clinic_send_token(uuid) is
  'Token da instância APTA a enviar (conectada, com token, sem bloqueio 463). NULL = clínica não pode enviar. Fonte única: evita que o token venha de uma instância diferente da que passou no gate.';

create or replace function public.fn_clinic_can_send(p_clinic_id uuid)
returns boolean language sql stable set search_path to 'public' as $$
  select fn_clinic_send_token(p_clinic_id) is not null
$$;

-- Avaliação por CLÍNICA (não por linha candidata).
create or replace view public.v_clinic_send_state
with (security_invoker = on) as
  select c.id as clinic_id, fn_clinic_send_token(c.id) as send_token
    from clinics c;
...
drop function if exists public.fn_followup_candidates_confirmation(uuid);
create function public.fn_followup_candidates_confirmation(p_clinic_id uuid default null)
returns table (
  clinic_id uuid, appointment_id uuid, lead_id uuid, nome text, telefone text,
  data_consulta text, hora_consulta text, confirm_message text,
  eligible_at timestamp, toggle_on boolean, wa_ok boolean,
  window_start int, window_end int
)
language sql stable set search_path to 'public' as $$
...
returns table (
  ...
  outcome text, message text, encerrado_em timestamp,
...
exception when others then
  -- Sem isto o motor parava e ninguém ficava sabendo (rastro só em cron.job_run_details).
  perform log_system_error('forms-welcome','job_failed','Falha no job de boas-vindas','error',
...
declare
  ...
  v_max_per_clinic int := 5;
begin
  -- Vencimento é escrita, então fica no motor (a fonte única é só leitura).
  with expired as (
...
revoke all on function public.fn_clinic_send_token(uuid)                from public;
...
revoke all on public.v_clinic_send_state                                from public, anon, authenticated;
