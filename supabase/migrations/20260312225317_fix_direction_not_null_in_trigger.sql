-- 20260312225317_fix_direction_not_null_in_trigger
-- Exportado de supabase_migrations.schema_migrations em 11/08/2026 (backfill).
-- JA APLICADA em producao nesta versao. Arquivo existe para o repo poder reconstruir o banco.

-- Remover o default de sender para que o gatilho possa detectar quando o n8n o omite
ALTER TABLE public.chat_messages ALTER COLUMN sender DROP DEFAULT;

-- Garantir que a direção pode ser nula temporariamente, ou ter um valor fallback no gatilho
-- Opcional: ALTER TABLE public.chat_messages ALTER COLUMN direction DROP NOT NULL;
-- Vamos apenas atualizar o gatilho para ser mais robusto
...
BEGIN
    ...
    IF (NEW.message IS NOT NULL) THEN
        ...
        -- Forçar re-avaliação do sender e direction se foram gerados pelo sistema/n8n
        IF NEW.sender IS NULL OR NEW.sender = 'system' THEN
            ...
            BEGIN
                ...
                ELSE
                    NEW.sender := 'system';
                    NEW.direction := 'outbound'; -- Fallback
                END IF;
            END;
        END IF;
    END IF;
    ...
    -- Garantir que a direção nunca seja nula
    IF NEW.direction IS NULL THEN
        NEW.direction := 'outbound'; -- Fallback final para evitar o erro null constraint
    END IF;
