-- A chave da memoria do Agente IA passa a ter FONTE UNICA: quem grava a conversa e quem
-- entrega o turno ao agente usam a MESMA string.
--
-- Defeito corrigido (medido em 30/07/2026): o wa-inbound repassava o telefone CRU do chatid
-- (13 digitos, COM o 9) e o ai-agent montava a session_id com ele, enquanto a conversa era
-- gravada com normalize_br_phone (12 digitos, SEM o 9). Davam duas "pastas" para o mesmo
-- paciente: o agente lia a dele, onde so existiam as falas dele mesmo, e o
-- `while (turns[0].role==='assistant') shift()` do loadConversation apagava todas. O modelo
-- recebia UMA mensagem sem historico, se reapresentava e repergunta o nome. Clinica Vaz:
-- 49 de 59 leads com IA nos ultimos 14 dias.
--
-- Aqui a RPC passa a DEVOLVER a session_id que ela mesma usou (a mesma que o trigger
-- fn_fill_chat_session_id monta) e o telefone normalizado. O wa-inbound repassa, o ai-agent
-- usa. Nenhuma logica de normalizacao vira para o TypeScript de proposito: telefone e chave,
-- e a chave tem um dono so, que e o banco.
--
-- O endereco de ENTREGA (contact_identifier) continua CRU, como a uazapi devolveu (CLAUDE.md
-- 0.4): normalizar a chave e normalizar o destino sao coisas diferentes.
...
begin
  ...
  -- MESMA composicao do fn_fill_chat_session_id (clinic_phone || telefone normalizado). Sem
  -- telefone da instancia o trigger nao monta nada, e aqui tambem nao: devolver uma chave que
  -- a conversa nao usa seria voltar exatamente ao defeito que esta migration corrige.
  v_session_id := case
    when v_clinic_phone is not null and v_clinic_phone <> '' then v_clinic_phone || v_norm
    else null end;
