-- 20260707234129_commercial_leads_metric_count
...
-- (ex.: "Consultas Geradas = 7" mas "5 leads", pois 2 leads têm 2 agendamentos).
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(MAX(ma.cnt), 0)
  INTO v_total, v_rows, v_metric_count
  FROM page p
  CROSS JOIN metric_appts ma
  ...
  -- page pode estar vazio (0 leads) e o CROSS JOIN some — recalcula o count nesse caso
  IF v_metric_count IS NULL THEN
    SELECT COALESCE(cnt, 0) INTO v_metric_count FROM (
      SELECT 0 AS cnt WHERE p_metric = 'todos'
    ) z;
    v_metric_count := COALESCE(v_metric_count, 0);
  END IF;
